...
{
	if(xmlFile.indexOf("login.xml") > 0 || xmlFile.indexOf("mainview.xml") > 0){
		getXMLHandler("./xml/htmlalert.xml", lan);	//加载语言xml
		getJsAlertText();
	}
...
function TranslatePage()
{
	...
	TranslateElements(document, 'option', 'innerText');
...
if(m_xmlDoc != null){
	var tmp_xmlDoc = m_xmlDoc.getElementsByTagName("Js")[0].childNodes;
	for(var i = 0; i < tmp_xmlDoc.length; i++){
		if(tmp_xmlDoc[i].nodeName != null){
			try{
				eval("var " + tmp_xmlDoc[i].nodeName + " = '" + tmp_xmlDoc[i].childNodes[0].nodeValue + "';");
			}catch(e){}
		}
	}
	tmp_xmlDoc	= null;
	tmp_xmlDoc = m_xmlDoc.getElementsByTagName("Array")[0].childNodes;
	for(var i = 0; i < tmp_xmlDoc.length; i++){
		if(tmp_xmlDoc[i].nodeName != null){
			try{
				eval("var " + tmp_xmlDoc[i].nodeName + " = " + tmp_xmlDoc[i].childNodes[0].nodeValue + ";");
